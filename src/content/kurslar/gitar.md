---
title: "Gitar Dersi Kayseri | Arya Sanat Akademisi — Kocasinan"
description: "Kayseri Kocasinan gitar dersi — Arya Sanat Akademisi. Klasik, akustik ve elektro gitar eğitimi. Başlangıçtan ileri seviyeye bireysel dersler. MEB onaylı sertifika. Ücretsiz deneme dersi!"
keywords: "gitar dersi Kayseri, gitar kursu kocasinan, akustik gitar dersi, elektro gitar kursu, klasik gitar dersi kayseri, gitar öğrenmek"
badge: "🎸 Gitar Eğitimi (Elektro, Bass)"
heroTitle: "Gitar Dersi"
heroTitleHighlight: "(Elektro, Bass)"
heroDesc: "Klasik gitarın zarafetinden elektro gitarın enerjisine ve bass gitarın derin tonlarına — her stilin ustası olun. Berklee sertifikalı eğitmenimizle müzik yolculuğunuza başlayın."
image: "/assets/teacher_guitar.png"
duration: "45 dk / ders"
frequency: "Haftada 1–2 ders"
level: "Başlangıç → İleri"
certificate: "Klasik / Akustik / Elektro"
leadText: "Gitar, en evrensel enstrümanlardan biridir — ve doğru rehberlikle öğrenmesi de o kadar keyiflidir."
levels:
  beginner:
    title: "İlk Gitarım"
    desc: "Nasıl tutacağını bilmeyenlerden başlayarak sistemli ilerleme."
  intermediate:
    title: "Akorları Biliyorum"
    desc: "Teknik boşlukları doldurun, yeni stiller keşfedin."
  advanced:
    title: "Sahne Hedefim Var"
    desc: "Performans, doğaçlama ve kayıt teknikleri odaklı ileri program."
gallery:
  - "/assets/kurslar/gitar-kurs.jpg"
  - "/assets/kurslar/gitar-galeri-2.jpg"
  - "/assets/kurslar/gitar-galeri-3.jpg"
curriculum:
  - period: "1–3. Ay"
    title: "Temel Teknik ve Akorlar"
    items:
      - "Tutuş, parmak pozisyonu ve postür"
      - "Temel akordlar (Am, Em, C, G, D, F)"
      - "Basit strümleme kalıpları"
      - "Nota okuma ve perde sistemi"
  - period: "4–6. Ay"
    title: "Melodi ve Ritim Geliştirme"
    items:
      - "Barré akordlar ve kapodaster kullanımı"
      - "Fingerpicking (parmak tekniği) temelleri"
      - "Pentatonik ve majör gamlar"
      - "Şarkı repertuvarı oluşturma"
  - period: "7–10. Ay"
    title: "Stil Seçimi ve Derinleşme"
    items:
      - "Klasik: Klasik repertuvar, arpeji teknikleri"
      - "Akustik: Fingerstyle, blues ve folk"
      - "Elektro: Distortion, efektler, riff çalışma"
      - "Doğaçlama ve kulak eğitimi"
  - period: "11. Ay ve Sonrası"
    title: "İleri Teknikler ve Performans"
    items:
      - "Hammer-on, pull-off, slide, bend teknikleri"
      - "Şarkı yazarlığı ve kompozisyon"
      - "Grup müziği ve band çalışması"
      - "Sahne ve kayıt hazırlığı"
teacher:
  name: "Mehmet Yılmaz"
  specialty: "Gitar Eğitmeni"
  image: "/assets/teacher_guitar.png"
  bio1: "Berklee Online sertifikasına sahip olan Mehmet Yılmaz, 8 yıllık sahne ve stüdyo deneyimiyle rock, blues ve fingerstyle alanlarında uzmanlaşmıştır. Öğrencilerin kendi müzikal sesini bulmasına yardımcı olmayı temel hedef olarak belirlemektedir."
  bio2: "Çeşitli ulusal müzisyenlerle stüdyo çalışmaları yapan Mehmet, aynı zamanda özgün bestecilik alanında da aktif olarak üretmeye devam etmektedir."
  tags:
    - "Akustik"
    - "Elektro"
    - "Klasik"
    - "Fingerstyle"
    - "Blues"
faq:
  - question: "Hangi gitar türüyle başlamalıyım?"
    answer: "Bu tamamen hedeflerinize bağlıdır. Klasik ve fingerstyle için naylon telli klasik gitar; pop, rock ve folk için akustik; metal ve heavy rock için elektro gitar önerilir. İlk derste eğitmenimiz size en uygun seçeneği belirlemenize yardımcı olur."
  - question: "Gitar almam gerekiyor mu?"
    answer: "Dersler akademimizdeki enstrümanlarla yapılabilir. Ev çalışması için orta vadede bir gitar almanız önerilir — eğitmenimiz bütçenize uygun seçenekler için rehberlik eder."
  - question: "Nota okumayı bilmeden gitar öğrenebilir miyim?"
    answer: "Evet! Nota okuma paralel olarak öğretilir. Birçok popüler gitar stilinde tab (tablature) sistemi de kullanılır. Nota okuma zorunlu değil ama ilerlemenizi hızlandırır."
  - question: "Parmak ucum acımaz mı?"
    answer: "Başlangıçta hafif hassasiyet normal olmakla birlikte, doğru teknikle bu kısa sürede geçer. Naylon telli klasik gitarla başlamak parmak ucunuzu korumak için iyi bir seçenektir."
---
Arya Sanat Akademisi'ndeki gitar programımız, üç ana dal üzerine yapılandırılmıştır: Klasik Gitar, Akustik Gitar ve Elektro Gitar. Hangi stili seçerseniz seçin, sağlam bir teknik temel üzerine inşa edilen, kişiselleştirilmiş bir öğrenme deneyimi sizi bekliyor.

Berklee Online sertifikasına sahip ve 8 yıllık sahne deneyimine sahip eğitmenimiz Mehmet Yılmaz; fingerstyle, pena tekniği, akor teorisi, doğaçlama ve şarkı yazarlığı konularında rehberlik eder. Dersler hem nota okuma hem de kulak eğitimini dengeli biçimde içerir.

Eğitimde kullandığımız gitarlar arasında Taylor akustik, Gibson elektro ve Yamaha klasik gitarlar bulunmaktadır. Kendi gitarınızı getirebilir ya da akademimizdeki enstrümanları kullanabilirsiniz.
